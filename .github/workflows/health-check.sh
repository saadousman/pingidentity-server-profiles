...
check_statefulset_readiness() {
    ...
    while true; do
        ...
        # Get number of desired and ready replicas
        local desired_replicas=$(kubectl get statefulset "$statefulset_name" -n "$namespace" -o jsonpath='{.status.replicas}' 2>/dev/null)
        local ready_replicas=$(kubectl get statefulset "$statefulset_name" -n "$namespace" -o jsonpath='{.status.readyReplicas}' 2>/dev/null)

        # Default empty values to zero
        ready_replicas=${ready_replicas:-0}
        desired_replicas=${desired_replicas:-0}

        # Check all individual pods
        local not_ready_pods=$(kubectl get pods -n "$namespace" -l statefulset.kubernetes.io/pod-name="$statefulset_name" \
            -o json | jq '[.items[] | select(.status.conditions[] | select(.type=="Ready" and .status!="True"))] | length')

        if [ "$ready_replicas" -eq "$desired_replicas" ] && [ "$not_ready_pods" -eq 0 ]; then
            echo "StatefulSet '$statefulset_name' is fully ready. Ready replicas: $ready_replicas."
            return 0
        fi

        echo "Waiting for all pods to become ready... Ready: $ready_replicas, Desired: $desired_replicas, Not Ready: $not_ready_pods"
        sleep 5
    done
}
